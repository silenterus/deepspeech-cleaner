...
def show_progress(current,maxlen):
    os.system('clear')
    print('')
    print('')
    print('                       WORKING')
...
def insert_db(all_wavdata):
    ...
    for wavdata in all_wavdata:
        ...
        sql_command = """insert or ignore into audios(size,duration,samples,wordcount,quality,distance,wav_path,gender,age,speaker,text,samplerate,bitrate,channels,codec,words_per_sec,dialect,dataset,upvotes,downvotes,errors,language) values(""" + str(wavdata[0]) + ',' + str(wavdata[1])  + ',' + str(wavdata[2]) + ',' + str(wavdata[3]) + ',\'' + str(wavdata[4]) + '\',\'' + str(wavdata[5]) + '\',\'' + str(wavdata[6]).replace('/home/silenter/deepspeech-ger/','') + '\',\'' + str(wavdata[7]) + '\',\'' + str(wavdata[8]) + '\',\'' + str(wavdata[9]) + '\',\'' + str(wavdata[10]) + '\',' + str(wavdata[11]) + ',' + str(wavdata[12])  + ',' + str(wavdata[13])  + ',\'' + str(wavdata[14])  + '\',' + str(wavdata[15])  + ',\'' + str(wavdata[16])  + '\',\'' + str(wavdata[17]) + '\',' + str(wavdata[18]) + ',' + str(wavdata[19]) + ',' + str(wavdata[20]) + ',\'' + str(wavdata[21]) + '\',' + str(wavdata[22]) + ',' + str(wavdata[23]) + """)"""  
...
def create_train_files():
    ...
    #cursor.execute("""SELECT dataset,wav_path,size,text from audios where words_per_sec<1.1 and words_per_sec>0.19 and duration<9.9 and duration>1.5 and size>500  and errors='0' and size<400000 and text not like '%�%' and text not like '%_BAD%' and text not like '%BAD_%'  and wav_path not like '%_Yamaha.%'  and wav_path not like '%_Samson.%'  and wav_path not like '%_Realtek.%'""")
...
def create_train_files():
    ...
    cursor.execute("""SELECT dataset,wav_path,size,text,duration from audios where errors=0 and lang='""" + str(lang) + """'""" + str(query) + """""")
    data = cursor.fetchall()
    olddata = True
    failer = 0  
    notfound = 0                    
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='DeepSpeech Dataset Cleaner')
    parser.add_argument('mode',help="create_files - [create training files from db],\n\ninsert_cv - [insert Common Voice Corpus in db],\n\ndelete_model - [delete training folder (containing all files and exported graphs)],\n\ndelete_checkpoints - [delete checkpoints  (containing all files and exported graphs)],\n\ncreate_trie - [creating trie file],\n\ncreate_lm - [creating lm.binary file],\n\ncreate_trie_lm - [creating trie and lm.binary file]",type=str)
    parser.add_argument('--training',help=" [%(default)s] path for model files", default='defaulter', type=str)
    parser.add_argument('--size',help="select min-max file size - 0=all \'example : 300-100000\'\n\n[%(default)s]", default='300-450000', type=str)
    ...
    if mode == 'create_files':
        ...
        corpus_path = input("enter cv corpus path : ")
        if os.path.isdir(current_dir + '/training/' + current_model) == False:
            deepspeech_parser(corpus_path)
        else:
            print('   ¦¦!¦¦ [' + str(corpus_path) + '] not found')
...
def create_new_db(): 
    if os.path.isfile("audio.db") == True:
        ...
        if deepspeech_data[0][0] == 'default':
            ...
            sql_command = """update configs set deepspeech_dir='""" + str(deepspeech_path) + """' where configs_id=""" + str(deepspeech_data[0][1]) + """"""  
...
def create_tri(sentences,changer):
    ...
    f.write("\n".join(sentences))
    f.close()
   # os.system("rm " + current_dir + "/training/" + current_model + "/all_clean_new && "  + "rm " + current_dir + "/training/" + current_model + "/all_clean_newer")



    # creating arpa, tri, lm
