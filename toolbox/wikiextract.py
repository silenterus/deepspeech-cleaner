...
def dump_xml(input_file='',path_to='',processes=0):


    real_p = max(1, cpu_count() - 1)
    if processes < 0:
        processes = 1
    elif processes == 0 or processes > real_p:
        processes = real_p
    ...
    try:
        power = 'kmg'.find(str('10000M')[-1].lower()) + 1
        file_size = int(str('10000M')[:-1]) * 1024 ** power
        if file_size < minFileSize:
            raise ValueError()
    except ValueError:
        logging.error('Insufficient or invalid size: %s', str('200M'))
        return
    ...
    tools.printer(2,'processing',processes)
